...
const rz = () => donutChart?.resize()
onMounted(() => { nextTick(renderDonut); window.addEventListener('resize', rz) })
watch(() => store.selectedUnitId, () => nextTick(renderDonut))
onUnmounted(() => { window.removeEventListener('resize', rz); donutChart?.dispose() })
</script>

<style scoped>
.ag-page { display: flex; flex-direction: column; gap: 12px; height: 100%; overflow: hidden; }
.ag-head { display: flex; align-items: baseline; gap: 16px; flex-wrap: wrap; flex-shrink: 0; }
...
.g-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; align-items: start; flex: 1; min-height: 0; overflow: auto; }
.g-card {
...
/* 统计 */
.ag-stat-card { padding-bottom: 8px; flex-shrink: 0; }
