...
<div class="bs">
    ...
    <div class="bs-up-cards">
      <div v-for="u in store.units" :key="u.id" class="bs-up-card" :class="{on: store.selectedUnitId===u.id}" @click="switchUnit(u.id)">
        ...
        <div class="bs-up-stats">
          ...
          <div class="bs-up-stat"><span>报警</span><strong :style="{color: store.unitAlarms(u.id).length > 0 ? '#ef4444' : '#94a3b8'}">{{ store.unitAlarms(u.id).length }}</strong></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <el-row :gutter="12" class="bs-row">
    ...
    <el-col :span="4"><div class="kpi"><div class="kpi-l">设备健康度</div><div class="kpi-v" :style="{color: avgHealth >= 90 ? '#22c55e' : avgHealth >= 80 ? '#f59e0b' : '#ef4444'}">{{ avgHealth }}%</div></div></el-col>
  </el-row>
        ...
        <table class="tbl">
          ...
          <tbody>
            <tr v-for="d in topWarn" :key="d.id">
              <td>{{ d.name }}</td>
              <td :style="{color: d.health < 70 ? '#ef4444' : d.health < 80 ? '#f59e0b' : '#f59e0b'}">{{ d.problem }}</td>
...
const insights = computed(() => {
  ...
  if (emCheck.length === 0) out.push({ type: 'info', icon: '✅', title: '环保排放达标', content: `NOx/SO2/粉尘均在国标限值 70% 以下，达标率 ${(99 + Math.random()).toFixed(1)}%。建议保持当前燃烧参数。` })
...
<style scoped>
.bs { min-height: 100vh; padding: 14px; background: linear-gradient(180deg, #020408, #05070b); }
...
/* 机组选择板块（紧凑大屏风格） */
.bs-unit-panel { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 8px 14px; background: rgba(10,13,22,0.35); border: 0.5px solid rgba(59,130,246,0.25); border-radius: 8px; }
...
.bs-up-card { display: flex; align-items: center; gap: 12px; padding: 6px 12px; background: #05070b; border: 1px solid #0f141d; border-radius: 6px; cursor: pointer; transition: 0.15s; min-width: 180px; position: relative; }
...
.kpi { background: rgba(10,13,22,0.42); border: 0.5px solid rgba(59,130,246,0.25); border-radius: 8px; padding: 14px; text-align: center; }
.kpi-l { font-size: 12px; color: #94a3b8; margin-bottom: 6px; }
.kpi-v { font-size: 28px; font-weight: 600; }
.cd { background: rgba(10,13,22,0.35); border: 0.5px solid #0f141d; border-radius: 8px; padding: 14px; height: 100%; }
