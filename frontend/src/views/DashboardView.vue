...
<div class="dash-wrap">
  ...
  <div class="dash-metrics-row">
    <div class="cd dash-metric-panel">
      <div class="cd-t"><span class="ut-ic">▸</span><span class="sd" :class="overviewStatus==='需关注'?'wn':'ok'"></span>{{ store.selectedUnit.name }} 机组实时概览<span class="tg" :class="overviewStatus==='需关注'?'tg-w':'tg-ok'">{{ overviewStatus }}</span></div>
    ...
    <div class="cd dash-metric-panel">
      <div class="cd-t"><span class="ut-ic">▸</span>环保排放指标<span class="tg" :class="emissionOk?'tg-ok':'tg-dg'">{{ emissionOk?'达标':'超标' }}</span></div>
      <div class="metric-grid">
  ...
  <!-- AI洞察 -->
  <div class="cd insight">
    <div class="cd-t"><span class="ut-ic">▸</span>AI 实时运行洞察 · {{ store.selectedUnit.name }} <span class="tg tg-ok">实时分析中</span></div>
...
const rz=()=>{c1?.resize();c2?.resize();c3?.resize()}
onMounted(()=>{nextTick(()=>{initCharts();updateMetrics()});window.addEventListener('resize',rz)})
iv=setInterval(updateMetrics,3000)
onUnmounted(()=>{clearInterval(iv);window.removeEventListener('resize',rz);c1?.dispose();c2?.dispose();c3?.dispose()})
</script>
...
/* ================================================================
   ...
   ================================================================ */
.dash-wrap { display:flex; flex-direction:column; gap:10px; height:100%; overflow:hidden; }
...
.up-card {
  ...
  background:linear-gradient(180deg, rgba(4,10,20,0.42), rgba(3,8,16,0.455));
...
.dept-switch { flex-shrink:0; display:flex; align-items:center; gap:10px; padding:8px 14px; background:linear-gradient(180deg, rgba(4,10,20,0.315), rgba(3,8,16,0.35)); border:1px solid rgba(62,170,255,0.1); border-radius:5px; }
.dept-lbl { font-size:12px; color:#8fb0cf; }
:deep(.dept-switch .el-radio-button__inner) { background:rgba(4,9,16,0.35); border-color:rgba(62,170,255,0.12); color:#8fb0cf; }
...
/* 指标面板 */
.dash-metrics-row { flex-shrink:0; display:grid; grid-template-columns:1fr 1fr; gap:12px; }
.metric-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; padding:12px 8px 8px; }
.mc { background:rgba(4,9,16,0.35); border-radius:5px; padding:12px 8px; text-align:center; border:1px solid rgba(62,170,255,0.06); transition:all 0.2s; }
.mc:hover { border-color:rgba(62,170,255,0.2); background:rgba(4,10,20,0.455); }
...
/* 健康度 + 图表混合行 */
.dash-health-row { display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:12px; flex:1; min-height:0; overflow:hidden; }
.health-col { min-height:0; display:flex; flex-direction:column; overflow:hidden; }
.chart-col { min-height:0; display:flex; flex-direction:column; overflow:hidden; }
.hb-list { flex:1; overflow:auto; padding:8px 4px; display:flex; flex-direction:column; gap:6px; }
...
.dash-chart-sm { flex:1; min-height:0; }
...
.in-problems { margin-top:10px; padding-top:10px; border-top:1px dashed rgba(62,170,255,0.12); overflow:auto; }
