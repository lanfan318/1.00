...
const defaultDiagnosis = (d) => {
  ...
  if (h >= 90) {
    return {
      ...
      analysis: `当前 ${d.name}（${d.dept}）健康度 ${h.toFixed(1)}，运行状态良好，未触发深度诊断分析。AI 将持续监控该设备的关键参数。`,
  ...
  if (h >= 80) {
    return {
      ...
      analysis: `${d.name}（${d.dept}）健康度 ${h.toFixed(1)}，处于需关注区间。建议结合历史趋势分析潜在风险。`,
  ...
  return {
    ...
    analysis: `${d.name}（${d.dept}）健康度仅 ${h.toFixed(1)}，已低于预警阈值 80。可能存在潜在故障，建议立即排查。`,
